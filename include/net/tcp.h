...
static inline void tcp_packets_out_inc(struct sock *sk, 
				       ...
				       const struct sk_buff *skb)
{
	int orig = tp->packets_out; // 已发送但未确认段的个数

	tp->packets_out += tcp_skb_pcount(skb);
	if (!orig)  // 定时器启动时机是必须是所有段都收到了确认
...
{
	const struct inet_connection_sock *icsk = inet_csk(sk);
	if (!tp->packets_out && !icsk->icsk_pending) // 此时网络上没有任何发送得报文 且 没有任何定时器(6个) 则启动0窗探测
...
static inline void tcp_done(struct sock *sk)  // 整个tcp连接完全断开 或 sock没有和任何进程相关联(SOCK_DEAD态) 或 收到reset时
{
