...
int register_qdisc(struct Qdisc_ops *qops) //注册排队规则 即连到qdisc_base链上 一种排队规则只能注册一次 默认排队规则fifo在pktsched_init中注册
{
 ...
 */
 //一个排队规则可以有多个类
static int qdisc_graft(struct net_device *dev, struct Qdisc *parent, //将一个规则绑定到一个类
		       u32 classid,
...
static int tc_modify_qdisc(struct sk_buff *skb, struct nlmsghdr *n, void *arg) //tc命令通过netlink修改qdisc  //skb netlink报文   n netlink报文首部 arg 配置排队规则消息的扩展属性
{
	struct tcmsg *tcm;
	struct rtattr **tca;
	struct net_device *dev;
...
replay:
	...
	if (clid) { //待配置排队规则存在父排队规则
		if (clid != TC_H_ROOT) { //获取父排队规则 如果是根排队规则 则直接从网卡上获取相应的排队规则 前者为qdisc_sleeping 后者为qdisc_ingress  ...
		...
		if (q && q->handle == 0) //若排队规则是默认排队规则 即认为当前没有配置排队规则
			q = NULL;
...
static int __init pktsched_init(void)
{
	...
	register_qdisc(&pfifo_qdisc_ops); //注册bfifo pfifo排队规则
