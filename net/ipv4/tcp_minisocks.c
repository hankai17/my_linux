...
/* Short-time timewait calendar */
	...
	.twcal_hand	= -1,
	.twcal_timer	= TIMER_INITIALIZER(inet_twdr_twcal_tick, 0,    // 短定时器 --> twcal_row(短定时器hash表)     recycle模式使用
					    (unsigned long)&tcp_death_row),
};
			   ...
			   const struct tcphdr *th)
{
	...
	if (tw->tw_substate == TCP_FIN_WAIT2) {                                     // fin2的定时器到期后tw态 此时收到了对端的包?
		...
		if (th->syn && !before(TCP_SKB_CB(skb)->seq, tcptw->tw_rcv_nxt))        // tw态收到握手包且seq小 则发rst
			goto kill_with_rst;
		 ...
		 */
		if (!th->fin ||
		    TCP_SKB_CB(skb)->end_seq != tcptw->tw_rcv_nxt + 1) {                // tw态收到数据包 则发rst
kill_with_rst:
...
kill:
		...
		inet_twsk_schedule(tw, &tcp_death_row, TCP_TIMEWAIT_LEN,                // 如果是ack包 则启动定时器后丢弃该ack
				   TCP_TIMEWAIT_LEN);
	...
	if (th->syn && !th->rst && !th->ack && !paws_reject &&              // 合法syn: 1)必须是syn 
		...
		return TCP_TW_SYN;                                              // tw态下收到了一个合法的syn包 首先查内核中是否有对应的监听套接字 如果有则会释放tw 开启新连接 若没有 则走TCP_TW_ACK分支
	}
...
void tcp_time_wait(struct sock *sk, int state, int timeo)
{
	...
	if (tcp_death_row.tw_count < tcp_death_row.sysctl_max_tw_buckets)
		tw = inet_twsk_alloc(sk, state);                                // 分配tw定时器对象 
	...
	if (tw != NULL) {
		...
		if (recycle_ok) {
			tw->tw_timeout = rto;                                   // recycle模式  3.5RTO
		} else {
			tw->tw_timeout = TCP_TIMEWAIT_LEN;                      // 2MSL=60s
