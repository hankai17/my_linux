			     ...
			     struct sk_buff *skb)
{
	...
	tcp_packets_out_inc(sk, tp, skb); // 里面启动发送重传超时定时器
}
...
static inline unsigned int tcp_cwnd_test(struct tcp_sock *tp, struct sk_buff *skb)
{
	...
	if (TCP_SKB_CB(skb)->flags & TCPCB_FLAG_FIN)                                        // 如果有fin 则返回1 即只能输出一个段
		return 1;

	in_flight = tcp_packets_in_flight(tp);
	cwnd = tp->snd_cwnd;
	if (in_flight < cwnd)
		return (cwnd - in_flight);                                                      // 扩大在途字节数
			       ...
			       unsigned int cur_mss, int nonagle)
{
	...
	if (skb) {
		if (tcp_write_xmit(sk, cur_mss, nonagle))
			tcp_check_probe_timer(sk, tp); // 此时网络上没有数据 但是write_queue队列不空(可能因为拥塞发不出去/接受窗口满/惰性窗口综合征SWS) 那么得检测是否是0窗口
	}
}
...
{
	struct tcp_sock *tp = tcp_sk(sk);	
	struct sk_buff *skb = skb_peek_tail(&sk->sk_write_queue);               // 取尾巴
	int mss_now;
	...
	if (sk->sk_send_head != NULL) {                                         // 如果发送队列不为空 buffer尾部设置sk fin标记位
		TCP_SKB_CB(skb)->flags |= TCPCB_FLAG_FIN;
