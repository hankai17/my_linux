 ...
 */
int dev_open(struct net_device *dev) //应用层ifconfig up (ioctl SIOCSIFFLAGS)激活设备
{
	int ret = 0;
	 ...
	 *	Is it even present?
	 */
	if (!netif_device_present(dev)) //whether 挂起
        return -ENODEV;
	...
	if (!ret) { //start ok
		 ...
		 */
		dev_mc_upload(dev); //更新组播地址列表到网络设备中 网络设备置传递态

		/*
		 *	Wakeup transmit queue engine
		 */
		dev_activate(dev); //初始化流控排队规则 & 启动监视定时器  默认流控为FIFO

		/*
		 *	... and announce new interface.
		 */
		raw_notifier_call_chain(&netdev_chain, NETDEV_UP, dev); //发送NETDEV_UP消息 到 网络设备状态改变通知连上
	}
	return ret;
}
...
int dev_close(struct net_device *dev)
{
	...
	while (test_bit(__LINK_STATE_RX_SCHED, &dev->state)) { //如果网络数据包在轮训接收数据包 则等待至接收完成
		/* No hurry. */
		msleep(1);
	}
...
static void dev_queue_xmit_nit(struct sk_buff *skb, struct net_device *dev) //用于原始套接字 读取本地输出数据包  读取本地输入数据包一般套接字就可以搞定
{
	...
	rcu_read_lock();
	list_for_each_entry_rcu(ptype, &ptype_all, list) { //只需遍历ptype_all链表即可 无需查找ptype_base hash表
...
void __netif_schedule(struct net_device *dev) //激活网卡输出软中断的函数很多 最常用的是netif_schedule
{
	if (!test_and_set_bit(__LINK_STATE_SCHED, &dev->state)) { //如果输出网卡没有处于流控调度中 则将该网卡连到softnet_data中的output_queue队列上 然后激活网卡输出软中断对该队列进行处理
		unsigned long flags;
...
int dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev) //输出数据包到网卡
{
	if (likely(!skb->next)) { //通常都是输出单独数据包
		if (netdev_nit) //如果应用层创建的是原生套接字 则需发送一份数据包给这样的套接口
			dev_queue_xmit_nit(skb, dev);

		if (netif_needs_gso(dev, skb)) { //如果待输出的数据包是GSO数据包 但网卡不支持 则调用dev_gso_segment对GSO数据包进行软分割 如果分割后仍是一个数据包 则直接调用网卡的hard_start_xmit输出数据包
			if (unlikely(dev_gso_segment(skb))) //然而通常一个GSO数据包软分割后 会产生多个连接起来的数据包 若这样则调到gso处 逐个处理数据包
				goto out_kfree_skb;
			if (skb->next)
				goto gso;
		}

		return dev->hard_start_xmit(skb, dev); //一个GSO数据包软分割后 会产生多个连接起来的数据包 逐个处理数据包 调网卡的hard_start_xmit(e100驱动中为 e100_xmit_frame)输出数据包
	}
...
static void net_tx_action(struct softirq_action *h) //数据包输出软中断的例程 一旦激活便遍历output_queueu队列中待处理的输出网卡 然后调qdisc_run在合适时机发送数据 数据包输出软中断通常由netif_schedule激活
{
	struct softnet_data *sd = &__get_cpu_var(softnet_data);

	if (sd->completion_queue) { //如果当前cpu的softnet_data中存在已完成输出待释放的数据包 则遍历completion_queue队列 释放该队列中所有的数据包
	...
	if (sd->output_queue) { //如果当前cpu的softnet_data中存在待处理输出的网卡 则遍历output_queue队列 调用qdisc_run发送数据包 | 再次调度数据包输出软中断在合适时机发送数据包
...
{
	struct softnet_data *queue = &__get_cpu_var(softnet_data); //获取所在cpu的softnet_data
	unsigned long start_time = jiffies;
	int budget = netdev_budget; //获取此次软中断的接收报文总配额
	void *have;

	local_irq_disable();

	while (!list_empty(&queue->poll_list)) { //遍历poll_list
		struct net_device *dev;

		if (budget <= 0 || jiffies - start_time > 1) //总额度用完 | 超时  则结束软中断 统计所在cpu软中断处理次数 最后开中断
			goto softnet_break;
		...
		if (dev->quota <= 0 || dev->poll(dev, &budget)) { //e100_poll
...
int dev_change_flags(struct net_device *dev, unsigned flags) //应用层ifconfig up (ioctl SIOCSIFFLAGS)激活设备 //ifconfig down(ioctl SIOCSIFFLAGS)关闭网络设备
{
	int ret;
...
int register_netdevice(struct net_device *dev) //注册网卡
{
